...
| Root package | `firecrawl` |
| Distribution name | `firecrawl` |
...
Alternatively, scope it — `with FirecrawlClient(...) as client:` closes the pool on exit.
...
Alternatively, scope it — `async with AsyncFirecrawlClient(...) as client:` closes the pool on exit.

`AsyncClient` (`firecrawl/async_client.py`) mirrors `Client` method for method, each endpoint method a coroutine. It takes the same keywords, except that each client accepts only its own transport and — where the **Async Type** column differs — only its own flavor.

`Client` and `AsyncClient` are aliases of `FirecrawlClient` and `AsyncFirecrawlClient` — the names tracebacks and `repr()` show; all four import from the root.
...
Every API group is a property on the client (e.g. `client.account`). Every constructor argument is optional and keyword-only. Sources: `firecrawl/client.py`, `firecrawl/async_client.py`:
...
| --- | --- | --- |
| `HttpClient` | `firecrawl.core` | protocol — `send(request: HttpRequest) -> HttpResponse` · `close()` |
| `AsyncHttpClient` | `firecrawl.core` | protocol — `async send(request: HttpRequest) -> HttpResponse` · `async aclose()` |
...
- **Case A — typed error.** The operation documents at least one error status, so `firecrawl/errors/` declares a union alias over the bodies those statuses map to — `RawError` is always its last arm, for any undocumented status — and `.error` is annotated with that alias. Narrow it with `isinstance`. The operation blocks name the alias and the status each arm maps from.
- **Case B — raw error.** The operation documents no error status; `.error` is `RawError` (`firecrawl/core/results.py`): `status_code: int` · `content: bytes` · `text(encoding="utf-8"): str` · `json(): Any` · `response: HttpResponse`.

Core runtime types (`firecrawl/core/`) — public members with their **declared types**, verbatim from source:

| Type | Public members | Source |
| --- | --- | --- |
| `ApiError` — raised by every parsed call; `.error` is a Case A alias from `firecrawl/errors/` or `RawError` | `error: E` · `status_code: int` · `response: HttpResponse` | `firecrawl/core/exceptions.py` |
| `ApiResult[T, E]` — returned by every raw call; the `Success[T] \| Failure[E]` union | `payload: T` (on `Success`) · `error: E` (on `Failure`) · `response: HttpResponse` (on both) | `firecrawl/core/results.py` |
| `RawError` | `status_code: int` · `content: bytes` · `text(encoding="utf-8"): str` · `json(): Any` · `response: HttpResponse` | `firecrawl/core/results.py` |
...
```python
from firecrawl.core import ApiError, RawError
from firecrawl.models import Agent402Error1
...
| **A trailing `request_options`** — keyword-only and optional, for per-call overrides such as a timeout or extra headers; every signature ends with it | here (`firecrawl/core/request_options.py`) |
...
| --- | --- |
| Client (root) | `firecrawl` |
| Operation controllers | `firecrawl.apis` |
| Models | `firecrawl.models` |
| Enums | `firecrawl.models.enums` |
| Unions | `firecrawl.models.unions`, `firecrawl.models` |
| Error aliases | `firecrawl.errors` |
| Core runtime (`ApiError`, `ApiResult`, `RawError`, …) | `firecrawl.core` |
...
**One environment, one server** (`firecrawl/server/server_config.py`). The spec declares a single environment, so no `environment` keyword exists; the base URL and its override point:
